...
<script>
    ...
    function d1on() {
        ...
        if (document.getElementsByClassName('D1-on')) {
            ...
            if (d1ison.length > 0) {
                // var url = "http://localhost/ledprojectapp/api/led/update.php?id=1&status=on";
                // $.getJSON(url, function(data) {
                //     console.log("led on");
                //     console.log(data);
                // });
                async function updated1() {
                    let response = await fetch('http://localhost/ledprojectapp/api/led/update.php?id=1&status=on');
                    let data = await response.json();
                    return data;
                }
                updated1().then(data => console.log(data));
            } else {
                // var url = "http://localhost/ledprojectapp/api/led/update.php?id=1&status=off";
                // $.getJSON(url, function(data) {
                //     console.log("led off");
                //     console.log(data);
                // });
                async function updated1() {
                    let response = await fetch('http://localhost/ledprojectapp/api/led/update.php?id=1&status=off');
                    let data = await response.json();
                    return data;
                }
                updated1().then(data => console.log(data));
